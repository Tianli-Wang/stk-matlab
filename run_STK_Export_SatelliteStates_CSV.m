...
%       GroundStations\ground_station_definitions.csv
%       GroundStations\PerGroundStation\*.csv
%       GroundStations\ground_station_states_combined.csv
%       export_metadata.mat
...
% 如果你已经先导出过一��? satellite_states_combined.csv，想避免重复建星座，直接用这段更合�?�：
...
EXPORT_GROUND_STATION_STATES      = toLogical(getConfigValue(cfg, 'EXPORT_GROUND_STATION_STATES', true));
...
ground_station_folder = fullfile(run_folder, 'GroundStations');
ground_station_per_station_folder = fullfile(ground_station_folder, 'PerGroundStation');
ground_station_definition_csv_path = fullfile(ground_station_folder, 'ground_station_definitions.csv');
ground_station_combined_csv_path = fullfile(ground_station_folder, 'ground_station_states_combined.csv');
...
firstSuccessfulSatelliteCsv = "";

% STK 侧按“卫星�?�为粒度��?次�?�导出完整时间段的状态，
% 这样可以避免逐时间步 COM 读取带来的�?�能��?��?��?
for i = 1:numSatsToExport
    ...
    try
        ...
        if strlength(firstSuccessfulSatelliteCsv) == 0
            firstSuccessfulSatelliteCsv = string(outFile);
        end
...
if EXPORT_GROUND_STATION_STATES
    fprintf('Exporting ground-station reference/state CSVs...\n');
    groundStationExport = exportGroundStationData( ...
        GS_Defs, ground_station_folder, ground_station_per_station_folder, ...
        ground_station_definition_csv_path, ground_station_combined_csv_path, ...
        StartTime, StopTime, time_step_val, firstSuccessfulSatelliteCsv);
else
    groundStationExport = makeEmptyGroundStationExportInfo( ...
        ground_station_folder, ground_station_per_station_folder, ...
        ground_station_definition_csv_path, ground_station_combined_csv_path);
end
...
metadata.GroundStationCount = numel(GS_Defs);
metadata.GroundStationNames = {GS_Defs.Name};
metadata.GroundStationExportEnabled = EXPORT_GROUND_STATION_STATES;
metadata.GroundStationFolder = groundStationExport.OutputFolder;
metadata.GroundStationPerGroundStationFolder = groundStationExport.PerGroundStationFolder;
metadata.GroundStationDefinitionCsvPath = groundStationExport.DefinitionCsvPath;
metadata.GroundStationCombinedCsvPath = groundStationExport.CombinedCsvPath;
metadata.GroundStationTimeSampleCount = groundStationExport.TimeSampleCount;
...
% 下面先汇总导出结果，再按��?追加“相对�?�度”和“可见�?�后处理”两类离线产物�??
fprintf('\nExport finished in %.2f s.\n', toc(export_timer));
fprintf('Per-satellite CSV folder:\n  %s\n', per_satellite_folder);
if EXPORT_GROUND_STATION_STATES
    fprintf('Ground-station CSV folder:\n  %s\n', groundStationExport.OutputFolder);
end
...
if RUN_VISIBILITY_POSTPROCESS
    ...
    % 这里把状��? CSV 交给 STK_Full_Simulation_CSV.m��?
    % 自动生成按时间步拆分��? visibility_step*.csv 文件��?
...
if EXPORT_GROUND_STATION_STATES && isfile(groundStationExport.CombinedCsvPath)
    fprintf('Ground-station combined CSV:\n  %s\n', groundStationExport.CombinedCsvPath);
end
end
...
for k = 1:numel(GS_Defs)
    ...
    facility.Position.AssignGeodetic(GS_Defs(k).Lat, GS_Defs(k).Lon, GS_Defs(k).AltKm);
end
end

function configureExportEnvironment(root)
% 固定导出单位��? CSV 格式，避免受 STK 当前界面设置影响��?
...
% 调用 STK ��? ReportCreate，一次�?�导出单颗卫星整个时间段的状态�??
...
% 将�?�卫��? CSV 追加汇�?�为 combined CSV，便于后续离线统��?读取��?
...
% ��? STK 原始导出表头统一改写��? Time_UTC / X_km / VX_km_s 等标准列名�??
fin = fopen(csvPath, 'r');
if fin == -1
...
function groundStationExport = exportGroundStationData( ...
    GS_Defs, outputFolder, perGroundStationFolder, definitionCsvPath, combinedCsvPath, ...
    StartTime, StopTime, timeStepSeconds, referenceSatelliteCsvPath)

% 这里把地面站导出拆成“定义表 + 状�?�表”两��? CSV，方便分别查看�??
% 状�?�表会按时间步展��?，并尽量复用卫星导出的时间轴，便于后续联表分析�??
ensureFolderExists(outputFolder);
ensureFolderExists(perGroundStationFolder);

timeKeys = resolveGroundStationTimeKeys(referenceSatelliteCsvPath, StartTime, StopTime, timeStepSeconds);
definitionTable = buildGroundStationDefinitionTable(GS_Defs);
writetable(definitionTable, definitionCsvPath);

stateTables = cell(numel(GS_Defs), 1);
for k = 1:numel(GS_Defs)
    stateTables{k} = buildGroundStationStateTable(GS_Defs(k), timeKeys);

    % 对单站文件名做一次清洗，避免名称中包含空格或特殊字符时影响文件读写�??
    oneStationCsvPath = fullfile(perGroundStationFolder, ...
        [makeSafeFileToken(GS_Defs(k).Name) '.csv']);
    writetable(stateTables{k}, oneStationCsvPath);
end

if isempty(stateTables)
    combinedTable = buildEmptyGroundStationStateTable();
else
    combinedTable = vertcat(stateTables{:});
end
writetable(combinedTable, combinedCsvPath);

groundStationExport = struct();
groundStationExport.OutputFolder = outputFolder;
groundStationExport.PerGroundStationFolder = perGroundStationFolder;
groundStationExport.DefinitionCsvPath = definitionCsvPath;
groundStationExport.CombinedCsvPath = combinedCsvPath;
groundStationExport.TimeSampleCount = numel(timeKeys);
end

function groundStationExport = makeEmptyGroundStationExportInfo( ...
    outputFolder, perGroundStationFolder, definitionCsvPath, combinedCsvPath)
...
groundStationExport.DefinitionCsvPath = definitionCsvPath;
groundStationExport.CombinedCsvPath = combinedCsvPath;
groundStationExport.TimeSampleCount = 0;
end

function timeKeys = resolveGroundStationTimeKeys(referenceSatelliteCsvPath, StartTime, StopTime, timeStepSeconds)
...
if strlength(referenceSatelliteCsvPath) > 0 && isfile(referenceSatelliteCsvPath)
    ...
    if ismember('Time_UTC', referenceTable.Properties.VariableNames)
        ...
        timeKeys = timeKeys(strlength(timeKeys) > 0);
        if ~isempty(timeKeys)
            timeKeys = timeKeys(:);
            return;
        end
...
% 如果卫星导出全部失败，就根据起止时间和步长回��?生成时间轴�??
timeKeys = buildTimeKeysFromRange(StartTime, StopTime, timeStepSeconds);
end

function timeKeys = buildTimeKeysFromRange(StartTime, StopTime, timeStepSeconds)
startDt = parseStkTimeString(StartTime);
stopDt = parseStkTimeString(StopTime);

if isnat(startDt) || isnat(stopDt)
    error('Unable to build ground-station time keys from StartTime/StopTime.');
end

if ~isfinite(timeStepSeconds) || timeStepSeconds <= 0
    timeKeys = string(cellstr(datestr(startDt, 'dd mmm yyyy HH:MM:SS.FFF')));
    return;
end

sampleTimes = (startDt:seconds(timeStepSeconds):stopDt)';
if isempty(sampleTimes)
    sampleTimes = startDt;
elseif sampleTimes(end) < stopDt
    sampleTimes(end + 1, 1) = stopDt;
end

timeKeys = string(cellstr(datestr(sampleTimes, 'dd mmm yyyy HH:MM:SS.FFF')));
timeKeys = timeKeys(:);
end

function dt = parseStkTimeString(timeText)
% STK 导出里常见的是英文月份格式，这里固定按英语月份缩写解析�??
dt = NaT;
timeText = string(timeText);
timeFormats = ["dd MMM yyyy HH:mm:ss.SSS", "dd MMM yyyy HH:mm:ss"];

for i = 1:numel(timeFormats)
    try
        dt = datetime(timeText, 'InputFormat', timeFormats(i), 'Locale', 'en_US');
        if ~isnat(dt)
            return;
        end
    catch
...
function T = buildGroundStationDefinitionTable(GS_Defs)
numGS = numel(GS_Defs);

groundStationNames = strings(numGS, 1);
objectTypes = repmat("Facility", numGS, 1);
latitudes = zeros(numGS, 1);
longitudes = zeros(numGS, 1);
altitudes = zeros(numGS, 1);
X = zeros(numGS, 1);
Y = zeros(numGS, 1);
Z = zeros(numGS, 1);

for k = 1:numGS
    groundStationNames(k) = string(GS_Defs(k).Name);
    latitudes(k) = GS_Defs(k).Lat;
    longitudes(k) = GS_Defs(k).Lon;
    altitudes(k) = GS_Defs(k).AltKm;

    gsEcef = geodeticToECEFkm(GS_Defs(k).Lat, GS_Defs(k).Lon, GS_Defs(k).AltKm);
    X(k) = gsEcef(1);
    Y(k) = gsEcef(2);
    Z(k) = gsEcef(3);
end
...
T = table( ...
    groundStationNames, objectTypes, latitudes, longitudes, altitudes, X, Y, Z, ...
    'VariableNames', {'GroundStation_Name', 'ObjectType', 'Latitude_deg', ...
    'Longitude_deg', 'Altitude_km', 'X_km', 'Y_km', 'Z_km'});
end

function T = buildGroundStationStateTable(gsDef, timeKeys)
numRows = numel(timeKeys);
gsName = repmat(string(gsDef.Name), numRows, 1);
objectTypes = repmat("Facility", numRows, 1);
latitudes = repmat(gsDef.Lat, numRows, 1);
longitudes = repmat(gsDef.Lon, numRows, 1);
altitudes = repmat(gsDef.AltKm, numRows, 1);

% 地面站在这里按地固坐标系静�?�建模，��?以位置恒定，速度显式写成 0��?
gsEcef = geodeticToECEFkm(gsDef.Lat, gsDef.Lon, gsDef.AltKm);
X = repmat(gsEcef(1), numRows, 1);
Y = repmat(gsEcef(2), numRows, 1);
Z = repmat(gsEcef(3), numRows, 1);
VX = zeros(numRows, 1);
VY = zeros(numRows, 1);
VZ = zeros(numRows, 1);

T = table( ...
    string(timeKeys(:)), gsName, objectTypes, latitudes, longitudes, altitudes, ...
    X, Y, Z, VX, VY, VZ, ...
    'VariableNames', {'Time_UTC', 'GroundStation_Name', 'ObjectType', ...
    'Latitude_deg', 'Longitude_deg', 'Altitude_km', ...
    'X_km', 'Y_km', 'Z_km', 'VX_km_s', 'VY_km_s', 'VZ_km_s'});
end

function T = buildEmptyGroundStationStateTable()
T = table( ...

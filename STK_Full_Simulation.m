...
for t_idx = 1:numTimeSteps
    ...
    visible_omegas = results_omega(mask); % <--- [新增]
...
for i = 1:8
   ...
   % 定义分类子文件夹
    ...
    for t_idx = 1:numTimeSteps
        ...
        for ii = 1:numSats
            if ~isempty(SatDataAll{ii})
                currentSatPositions(ii, :) = SatDataAll{ii}(t_idx, :);
                currentSatVelocities(ii, :) = SatVelAll{ii}(t_idx, :); % <--- [新增] 使用已提取的数据
            else
                currentSatPositions(ii, :) = [NaN, NaN, NaN];
                currentSatVelocities(ii, :) = [NaN, NaN, NaN];
            end
        end
        ...
        % --- C. 计算星间链路 (含角速度) ---
        results_dist = zeros(totalPairs, 1);
        results_omega = zeros(totalPairs, 1); % <--- [新增]
        results_visible = false(totalPairs, 1);
    ...
    fprintf('\n批量循环(Limit=%d)处理完成! 总耗时: %.2f 秒\n', distance_limit, toc(total_timer));
    fprintf('结果保存在: %s\n', output_folder);
end
